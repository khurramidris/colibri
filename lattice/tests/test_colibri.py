...
from lattice.colibri import clean_environment, detect_family, fingerprint_model, parse_calibration, parse_replay_metrics
...
class ColibriTests(unittest.TestCase):
    ...
    def test_model_fingerprint_uses_headers_and_metadata(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); (root / "config.json").write_text('{"model_type":"glm"}')
            (root / "tokenizer.json").write_text('{}'); shard = root / "a.safetensors"; write_safetensors(shard)
            before = fingerprint_model(root); data = shard.read_bytes(); shard.write_bytes(data[:-1] + b"\x01")
            self.assertNotEqual(before, fingerprint_model(root)); changed = fingerprint_model(root)
            shard.write_bytes(data + b"\x00"); self.assertNotEqual(changed, fingerprint_model(root))

    def test_out_of_bounds_tensor_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); (root / "config.json").write_text('{}'); (root / "tokenizer.json").write_text('{}')
            write_safetensors(root / "a.safetensors", {"x": {"dtype": "F32", "shape": [1], "data_offsets": [0, 999]}})
            with self.assertRaises(LatticeError): fingerprint_model(root)

    def test_clean_environment_removes_quality_and_adaptive_keys(self):
        env = clean_environment({"PATH": "/bin", "TOPK": "4", "PIPE": "1", "PIN": "auto"})
        self.assertNotIn("TOPK", env); self.assertNotIn("PIPE", env); self.assertNotIn("PIN", env)
    ...
    def test_parse_calibration_and_metrics(self):
        ...
        self.assertEqual(metrics["tok_s"], 2.5); self.assertEqual(metrics["hit_pct"], 70.5)
