...
class WorkspaceTests(unittest.TestCase):
    def test_immutable_run_is_idempotent_but_not_mutable(self):
        with tempfile.TemporaryDirectory() as directory:
            ws = Workspace(Path(directory)); ws.initialize({"schema_version": 1})
            run = {"schema_version": 1, "id": "run-abc"}; ws.write_run(run); ws.write_run(run)
            with self.assertRaises(LatticeError): ws.write_run({**run, "status": "changed"})
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with ws.acquire_lock("x"):
                with self.assertRaises(LatticeError):
                    with ws.acquire_lock("x"): pass

    def test_profile_pointer(self):
        with tempfile.TemporaryDirectory() as directory:
            ws = Workspace(Path(directory)); ws.initialize({"schema_version": 1})
            ws.write_profile({"schema_version": 1, "id": "profile-a"})
            self.assertEqual(ws.load_profile()["id"], "profile-a")

    def test_force_cannot_orphan_existing_evidence(self):
        with tempfile.TemporaryDirectory() as directory:
            ws = Workspace(Path(directory)); ws.initialize({"schema_version": 1, "name": "old"})
