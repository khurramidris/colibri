...
static size_t lt_find_any(const lt_scheduler_t *scheduler, uint64_t tensor_id) {
    size_t i;
    for (i = 0; i < scheduler->count; ++i) {
        if (scheduler->entries[i].request.tensor_id == tensor_id &&
            scheduler->entries[i].state != LT_REQ_CANCELLED) return i;
    }
    return (size_t)-1;
}
...
int lt_scheduler_submit(lt_scheduler_t *scheduler,
    ...
    found = lt_find_any(scheduler, request->tensor_id);
    if (found != (size_t)-1) {
...
int lt_scheduler_pop(lt_scheduler_t *scheduler,
    ...
    for (i = 0; i < scheduler->count; ++i) {
        ...
        if (cap != 0 && entry->request.bytes > cap - scheduler->stats.inflight_bytes) continue;
...
int lt_scheduler_complete(lt_scheduler_t *scheduler,
    ...
    if (!scheduler || tensor_id == 0) {
        lt_scheduler_error(error, error_cap, "invalid completion");
        return -1;
    }
    found = lt_find_any(scheduler, tensor_id);
    if (found == (size_t)-1) {
...
void lt_scheduler_compact(lt_scheduler_t *scheduler) {
    ...
    for (read_index = 0; read_index < scheduler->count; ++read_index) {
        ...
        if (state == LT_REQ_DONE || state == LT_REQ_CANCELLED) continue;
